...
def display_results(results):
    ...
    print("="*100)
    print(f"{'Stock':<8} {'Mean Return':<15} {'Median Return':<15} {'Std Dev':<15} {'5th %ile':<15} {'95th %ile':<15}")
    print("-"*100)
    
    # Sort by mean return
    sorted_results = sorted(results.items(), key=lambda x: x[1]['mean_annual_return'], reverse=True)
    
    for stock, metrics in sorted_results:
        print(f"{stock:<8} {metrics['mean_annual_return']:>13.2%} {metrics['median_annual_return']:>14.2%} "
...
def plot_top_stocks(results, top_n=10):
    ...
    for idx, (stock, metrics) in enumerate(top_stocks):
        axes[idx].hist(metrics['simulated_annual_returns'], bins=50, alpha=0.7, edgecolor='black')
        axes[idx].axvline(metrics['mean_annual_return'], color='red', linestyle='--', 
                         label=f"Mean: {metrics['mean_annual_return']:.2%}")
        axes[idx].axvline(metrics['median_annual_return'], color='green', linestyle='--', 
